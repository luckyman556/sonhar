...
abstract class EskilCore_Header {
	...
	public function __construct() {
		...
		add_action( 'eskil_core_action_before_main_css', array( $this, 'enqueue_assets' ) );
		add_action( 'eskil_core_action_before_main_css', array( $this, 'enqueue_additional_assets' ) );
		...
		add_filter( 'eskil_filter_add_inline_style', array( $this, 'set_inline_header_styles' ) );
		add_filter( 'eskil_filter_header_inner_class', array( $this, 'set_header_inner_classes' ), 10, 2 );
		add_filter( 'eskil_core_filter_nav_menu_header_selector', array( $this, 'set_nav_menu_header_selector' ) );
		add_filter( 'eskil_core_filter_nav_menu_narrow_header_selector', array( $this, 'set_nav_menu_narrow_header_selector' ) );
		add_filter( 'body_class', array( $this, 'add_body_header_classes' ) );
	}
	...
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'eskil_core_filter_header_template', $parameters );

		return eskil_core_get_template_part( 'header/layouts/' . $this->get_layout(), 'templates/' . $this->get_layout(), $this->get_layout_slug(), $parameters );
	}
	...
	public function get_header_transparency() {
		$background_color = eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_background_color' );
	...
	function set_header_height() {
		$header_height = eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;

		$this->header_height = apply_filters( 'eskil_core_filter_set_header_height', $header_height );
	}

	function set_header_inner_classes( $class, $layout ) {
		// Check is content in grid
		$class[] = 'yes' === eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_in_grid' ) ? 'qodef-content-grid' : '';
	...
	public function set_inline_header_styles( $style ) {
		...
		$height           = eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_height' );
		$background_color = eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_background_color' );
		...
		$side_padding = eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_side_padding' );
		$side_margin  = eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_side_margin' );
		$border_color = eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_color' );
		$border_width = eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_width' );
		$border_style = eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_style' );
		...
		if ( '' !== $side_margin ) {
			if ( qode_framework_string_ends_with_space_units( $side_margin ) ) {
				...
				if ( qode_framework_string_ends_with( $side_margin, 'px' ) ) {
					$inner_styles['width'] = 'calc(100% - ' . floatval( $side_margin ) * 2 . 'px)';
				} elseif ( qode_framework_string_ends_with( $side_margin, '%' ) ) {
					$inner_styles['width'] = 'calc(100% - ' . floatval( $side_margin ) * 2 . '%)';
				}
			} else {
	...
	public function add_body_header_classes( $classes ) {
		$border_color = eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_color' );
		$border_width = eskil_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_width' );
		...
		if ( ! empty( $border_color ) ) {
			...
			if ( ! empty( $border_width ) ) {
				$classes[] = 'qodef-header-border--enabled';
			}
		}
