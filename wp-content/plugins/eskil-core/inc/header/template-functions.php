...
if ( ! function_exists( 'eskil_core_get_header_logo_image' ) ) {
	/**
	 * This function print header logo image
	 *
	 * @param array $parameters
	 */
	function eskil_core_get_header_logo_image( $parameters = array() ) {
		$header_skin     = eskil_core_get_post_value_through_levels( 'qodef_header_skin' );
		$logo_height     = eskil_core_get_post_value_through_levels( 'qodef_logo_height' );
		$logo_source     = eskil_core_get_post_value_through_levels( 'qodef_logo_source' );
		$logo_svg_path   = eskil_core_get_post_value_through_levels( 'qodef_logo_svg_path' );
		$logo_text       = eskil_core_get_post_value_through_levels( 'qodef_logo_text' );
		$customizer_logo = eskil_core_get_customizer_logo();
		...
		if ( 'svg-path' === $logo_source && ! empty( $logo_svg_path ) ) {
			$logo_html['logo_main_image'] = apply_filters( 'eskil_core_filter_header_logo_svg_path', $logo_svg_path, $parameters );

			$is_enabled = true;
		} elseif ( 'textual' === $logo_source && ! empty( $logo_text ) ) {
			$logo_html['logo_main_image'] = esc_html( apply_filters( 'eskil_core_filter_header_logo_textual', $logo_text, $parameters ) );

			$is_enabled = true;
		} else {
			$available_logos = apply_filters(
				'eskil_core_filter_available_header_logo_images',
				array(
					'main'  => 'main',
					'dark'  => 'dark',
					'light' => 'light',
			...
			foreach ( $available_logos as $logo_key => $option_value ) {
				...
				$logo_image_id = eskil_core_get_post_value_through_levels( 'qodef_logo_' . $option_value );

				if ( empty( $logo_image_id ) && ! empty( $header_skin ) ) {
					$logo_image_id = eskil_core_get_post_value_through_levels( 'qodef_logo_main' );
				}
				...
				if ( ! empty( $logo_image_id ) ) {
					$logo_image_attr = array(
						...
						'itemprop' => 'image',
						'alt'      => sprintf( esc_attr__( 'logo %s', 'eskil-core' ), str_replace( '_', ' ', $logo_key ) ),
					);
		...
		$parameters['logo_image'] = implode( '', apply_filters( 'eskil_core_filter_header_logo_image_html', $logo_html, $parameters ) );

		if ( $is_enabled ) {
			echo apply_filters( 'eskil_core_filter_get_header_logo_image', eskil_core_get_template_part( 'header/templates', 'parts/logo', '', $parameters ), $parameters );
		}
	}
}

if ( ! function_exists( 'eskil_core_set_header_logo_styles' ) ) {
	/**
	 ...
	 * @return string
	 */
	function eskil_core_set_header_logo_styles( $style ) {
		$logo_styles  = array();
		$logo_height  = eskil_core_get_post_value_through_levels( 'qodef_logo_height' );
		$logo_padding = eskil_core_get_post_value_through_levels( 'qodef_logo_padding' );
		...
		// Logo SVG Source
		$svg_styles     = array();
		$svg_icon_color = eskil_core_get_post_value_through_levels( 'qodef_logo_svg_path_color' );
		...
		$svg_icon_size   = eskil_core_get_post_value_through_levels( 'qodef_logo_svg_path_size' );
		...
		$svg_icon_hover_color = eskil_core_get_post_value_through_levels( 'qodef_logo_svg_path_hover_color' );
		...
		// Logo Textual Source
		$textual_styles       = eskil_core_get_typography_styles( 'qodef_logo_text', '', null );
		$textual_hover_styles = eskil_core_get_typography_hover_styles( 'qodef_logo_text', null );
	...
	add_filter( 'eskil_filter_add_inline_style', 'eskil_core_set_header_logo_styles' );
}

if ( ! function_exists( 'eskil_core_get_header_widget_area' ) ) {
	/**
	 ...
	 * @param bool $enabled
	 */
	function eskil_core_get_header_widget_area( $widget_area = 'one', $widget_area_slug = 'header-widget-area', $custom_widget_area_slug = '', $enabled = false ) {
		...
		if ( $is_enabled || $enabled ) {
			...
			$parameters = apply_filters(
				'eskil_core_filter_header_widget_area',
				array(
					'page_id'             => $page_id,
			...
			eskil_core_template_part( 'header/templates', 'parts/widgets', '', $parameters );
		}
	}
}
