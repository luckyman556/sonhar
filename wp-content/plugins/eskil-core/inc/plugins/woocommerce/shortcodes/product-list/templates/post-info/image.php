...
$product_hover_image = get_post_meta( get_the_ID(), 'qodef_product_hover_image', true );
...
if ( $has_image ) {
	...
	echo eskil_core_get_list_shortcode_item_image( $image_dimension, $product_list_image, $custom_image_width, $custom_image_height );

	if ( ! empty( $product_hover_image ) ) {
		?>
		<div class="qodef-m-hover-image">
			<?php echo eskil_core_get_list_shortcode_item_image( $image_dimension, $product_hover_image, $custom_image_width, $custom_image_height ); ?>
		</div>
		<?php
	}
}