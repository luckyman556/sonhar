<?php if ( isset( $media ) && ! empty( $media ) ) {
	...
	foreach ( $images as $image ) {
		if ( isset( $image ) && ! empty( $image ) && is_array( wp_get_attachment_image_src( $image ) ) ) {
			$image_title     = get_the_title( $image );
			$image_src       = wp_get_attachment_image_src( $image, 'full' );
			$image_size_meta = eskil_core_get_custom_image_size_meta( 'attachment', 'qodef_image_portfolio_masonry_size', $image );
			?>
