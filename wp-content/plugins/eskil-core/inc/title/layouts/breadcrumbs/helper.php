...
if ( ! function_exists( 'eskil_core_register_breadcrumbs_title_layout' ) ) {
	/**
	 ...
	 * @return array
	 */
	function eskil_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'EskilCore_Breadcrumbs_Title';

		return $layouts;
	}

	add_filter( 'eskil_core_filter_register_title_layouts', 'eskil_core_register_breadcrumbs_title_layout' );
}

if ( ! function_exists( 'eskil_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function eskil_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'eskil-core' );

		return $layouts;
	}

	add_filter( 'eskil_core_filter_title_layout_options', 'eskil_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'eskil_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function eskil_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();

		// Breadcrumbs label
		$labels = apply_filters(
			'eskil_core_filter_breadcrumbs_label',
			array(
				'home'        => esc_html__( 'Home', 'eskil-core' ),
				'tag'         => esc_html__( 'Posts tagged "%s"', 'eskil-core' ),
				'author'      => esc_html__( 'Posted by %s', 'eskil-core' ),
				'search'      => esc_html__( 'Search results for "%s"', 'eskil-core' ),
				'404'         => esc_html__( '404 - Page not found', 'eskil-core' ),
				'query_paged' => esc_html__( '(Page %s)', 'eskil-core' ),
			)
		);

		// Breadcrumbs variables
		$settings = apply_filters(
			'eskil_core_filter_breadcrumbs_settings',
			array(
		...
		if ( is_home() && ! is_front_page() ) {
			...
			if ( is_tag() ) {
				...
				$wrap_child .= sprintf( $settings['current_item'], single_term_title( '', false ) );
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'eskil_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}

		echo apply_filters( 'eskil_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}
