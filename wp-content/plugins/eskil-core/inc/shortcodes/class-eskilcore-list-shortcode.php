...
abstract class EskilCore_List_Shortcode extends QodeFrameworkShortcode {
	private $post_type;
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'behavior', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => eskil_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => eskil_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
					'default_value' => '3',
					'group'         => $group,
					'dependency'    => array(
						'hide' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Row Height', 'eskil-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Max Row Height', 'eskil-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Last Row Treshold', 'eskil-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option(
			array(
				...
				'title'      => esc_html__( 'Enable Centered Slider', 'eskil-core' ),
		...
		$this->set_option(
			array(
				'field_type'  => 'text',
				'name'        => 'slider_speed',
				'title'       => esc_html__( 'Slide Duration', 'eskil-core' ),
				'description' => esc_html__( 'Default value is 5000 (ms)', 'eskil-core' ),
				'dependency'  => $dependency,
				'group'       => $group,
			)
		);
		$this->set_option(
			array(
				...
				'title'       => esc_html__( 'Slide Animation Duration', 'eskil-core' ),
				'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 800.', 'eskil-core' ),
				'dependency'  => $dependency,
				'group'       => $group,
			)
		);
		$this->set_option(
			array(
				...
				'title'      => esc_html__( 'Choose Slide Animation', 'eskil-core' ),
				'options'    => array(
					''     => esc_html__( 'Default (Slide)', 'eskil-core' ),
					'fade' => esc_html__( 'Fade', 'eskil-core' ),
				),
				'dependency' => $dependency,
				'group'      => $group,
			)
	...
	public function get_list_classes( $atts ) {
		...
		if ( isset( $atts['slider_pagination'] ) && 'no' !== $atts['slider_pagination'] ) {
			$holder_classes[] = 'qodef-swiper-pagination--on';
		}
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && 'masonry' === $atts['behavior'] && ! empty( $atts['masonry_images_proportion'] ) && 'fixed' === $atts['masonry_images_proportion'] ) {
			...
			$image_dimension              = eskil_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}
	...
	public function get_slider_data( $atts, $include = array() ) {
		...
		$data['spaceBetween']   = isset( $atts['space'] ) ? ( eskil_core_get_space_value( $atts['space'] ) * 2 ) : 0; // double half space for slider
		$data['centeredSlides'] = isset( $atts['slider_centered'] ) ? 'yes' === $atts['slider_centered'] : false;
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'eskil-core' );
		...
		$this->set_option(
			array(
				'field_type'    => 'text',
				'name'          => 'posts_per_page',
				'title'         => esc_html__( 'Posts per Page', 'eskil-core' ),
				'default_value' => '9',
				'group'         => $group,
			)
		);
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'orderby', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'orderby',
					'title'         => esc_html__( 'Order By', 'eskil-core' ),
					'options'       => eskil_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
					'default_value' => 'date',
					'group'         => $group,
				)
			);
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'order', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'order',
					'title'         => esc_html__( 'Order', 'eskil-core' ),
					'options'       => eskil_core_get_select_type_options_pool( 'order' ),
					'default_value' => 'DESC',
					'group'         => $group,
				)
			);
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'additional_params', $exclude_option, true ) ) ) {
			...
			do_action( 'eskil_core_action_map_query_options_before_additional', $group );

			$additional_params = apply_filters(
				'eskil_core_filter_map_additional_query_params',
				array(
					''       => esc_html__( 'No', 'eskil-core' ),
					'id'     => esc_html__( 'Post IDs', 'eskil-core' ),
					'tax'    => esc_html__( 'Taxonomy Slug', 'eskil-core' ),
					'author' => esc_html__( 'Author Name', 'eskil-core' ),
				),
				$this->get_base()
			);
			...
			$this->set_option(
				array(
					'field_type'  => 'text',
					'name'        => 'post_ids',
					'title'       => esc_html__( 'Posts IDs', 'eskil-core' ),
					'description' => esc_html__( 'Separate post IDs with commas', 'eskil-core' ),
					'group'       => $group,
					'dependency'  => array(
						'show' => array(
			...
			if ( ! empty( $taxonomies_formatted ) ) {
				$this->set_option(
					array(
						'field_type' => 'select',
						'name'       => 'tax',
						'title'      => esc_html__( 'Taxonomy', 'eskil-core' ),
				...
				$this->set_option(
					array(
						'field_type' => 'text',
						'name'       => 'tax_slug',
						'title'      => esc_html__( 'Taxonomy Slug', 'eskil-core' ),
						'group'      => $group,
						'dependency' => array(
							'show' => array(
								'additional_params' => array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'author_slug',
					'title'      => esc_html__( 'Author Slug', 'eskil-core' ),
					'group'      => $group,
					'dependency' => array(
						'show' => array(
							'additional_params' => array(
			...
			do_action( 'eskil_core_action_map_query_options_after_additional', $group );
		}
	}
	...
	public function get_additional_query_args( $atts ) {
		...
		$args = apply_filters( 'eskil_core_filter_additional_query_args', $args, $atts, $this->get_post_type() );
	...
	public function map_layout_options( $params = array() ) {
		...
		$this->set_option(
			array(
				...
				'options'       => $layouts,
				'default_value' => apply_filters( 'eskil_core_filter_map_layout_options_default_value', $default_value, $this->get_base(), $params ),
	...
	public function map_additional_options( $params = array() ) {
		$group_name         = esc_html__( 'Additional', 'eskil-core' );
		...
		if ( ! $exclude_filter ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'enable_filter',
					'title'         => esc_html__( 'Enable Filter', 'eskil-core' ),
					'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'eskil-core' ),
					'options'       => eskil_core_get_select_type_options_pool( 'no_yes' ),
					'default_value' => '',
					'dependency'    => array(
						'show' => array(
			...
			do_action( 'eskil_core_action_map_additional_options_after_filter', $group_name );
		}
		...
		if ( ! $exclude_pagination ) {
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Load More Top Margin (px or %)', 'eskil-core' ),
					'default_value' => '',
					'group'         => $group_name,
					'dependency'    => array(
						'show' => array(
			...
			do_action( 'eskil_core_action_map_additional_options_after_pagination', $group_name );
		}
	}
	...
	public function register_list_scripts() {
		...
		$list_scripts = apply_filters( 'eskil_core_filter_register_list_shortcode_scripts', isset( $scripts ) ? $scripts : array() );

		$this->set_scripts( $list_scripts );
	}

	public function load_assets() {
		do_action( 'eskil_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}
