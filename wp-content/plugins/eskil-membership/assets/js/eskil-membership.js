(function ( $ ) {
	...
	var qodefLoginModal = {
		...
		triggerShowModal: function ( $holder ) {
			...
			$( document.body ).on(
				'eskil_membership_trigger_login_modal',
				function () {
		...
		triggerRequest: function ( $holder, socialNetwork, socialResponse ) {
			...
			var ajaxData = {
				...
				nonce: $holder.find( '#eskil-membership-ajax-' + $requestType + '-nonce' ).val()
			};
		...
		triggerForceLogin: function ( $holder, ajaxData, redirect ) {
			...
			ajaxData.nonce                = $holder.parent().find( '#eskil-membership-ajax-login-nonce' ).val();
		...
		triggerClick: function ( $holder ) {
			...
			$opener.on(
				...
				function ( e ) {
					...
					$( document.body ).trigger( 'eskil_membership_trigger_login_modal' );
				}
			);
		}
	};
	...
	var qodefGoogleLogin = {
		init: function () {
			...
			if ( qodefGoogleLogin.isAppIdSet() ) {
				qodefGoogleLogin.asyncInit( eskilMembershipGlobal.googleAppId );
			}
		},
		isAppIdSet: function () {
			return typeof eskilMembershipGlobal.googleAppId !== 'undefined' && eskilMembershipGlobal.googleAppId !== '';
		},
		...
		login: function ( $modal, $form, social ) {
			...
			window.auth2.signIn().then(
				qodefGoogleLogin.signIn( $modal, $form, social ),
				function ( e ) {
					console.log( e );
				}
			);
