...
if ( ! class_exists( 'EskilMembership_Page_Templates' ) ) {
	class EskilMembership_Page_Templates {
		...
		public function __construct() {
			...
			$this->set_templates(
				array(
					'user-dashboard.php' => esc_html__( 'Qode User Dashboard', 'eskil-membership' ),
				)
			);
			...
			add_filter( 'eskil_membership_filter_dashboard_template_name', array( $this, 'set_dashboard_template_name' ) );

			// Include dashboard page template content when user is logged in
			add_action( 'eskil_membership_action_after_user_dashboard_page_content', array( $this, 'include_dashboard_template_content' ) );
		}

		/*
		 * @return EskilMembership_Page_Templates
		 */
		...
		public function get_template_path( $template ) {
			...
			if ( isset( $post ) && ! empty( $post ) && ! is_search() ) {
				...
				$file = ESKIL_MEMBERSHIP_INC_PATH . '/general/page-templates/' . $page_template;
		...
		function include_dashboard_template_content() {
			...
			if ( is_user_logged_in() ) {
				eskil_membership_template_part( 'general', 'page-templates/parts/content' );
			}
		}
	}

	EskilMembership_Page_Templates::get_instance();
}
