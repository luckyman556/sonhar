...
if ( ! class_exists( 'EskilMembershipTwitterApi' ) ) {
	/**
	 * Class EskilMembershipTwitterApi
	 */
	class EskilMembershipTwitterApi {
		/**
		 * @var $instance EskilMembershipTwitterApi class object
		 */
		...
		/**
		 * @return EskilMembershipTwitterApi
		 */
		 ...
		 *
		 * @see EskilMembershipTwitterApi::generateSignatureBase()
		 */
		...
		public function obtainRequestToken() {
			...
			if ( is_wp_error( $response ) ) {
				...
				$responseObj->message = esc_html__( 'Internal WP error', 'eskil-membership' );
			} else {
				...
				if ( ! empty( $responseBody ) ) {
					...
					if ( ( is_array( $responseParsed ) && count( $responseParsed ) ) && ! empty( $responseParsed['oauth_token'] ) && ! empty( $responseParsed['oauth_token_secret'] ) ) {
						...
						if ( empty( $responseObj->redirectUrl ) ) {
							...
							$responseObj->message                  = esc_html__( 'Redirect URL could not be generated', 'eskil-membership' );
						} else {
							$responseObj->oauth_callback_confirmed = true;
						}
					} else {
						$responseObj->oauth_callback_confirmed = false;
						$responseObj->message                  = esc_html__( 'Couldn\'t connect with Twitter API', 'eskil-membership' );
					}
				}
			}
		...
		public function buildCurrentPageURI() {
			...
			return eskil_membership_get_membership_redirect_url();
		}
	}
}
