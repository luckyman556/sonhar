...
class Qode_Variation_Swatches_For_WooCommerce_Framework_WPBakery_Translator {

	public function __construct() {
		$this->add_shortcodes();

		add_filter( 'qode_variation_swatches_for_woocommerce_filter_framework_shortcode_content_html', array( $this, 'format_shortcode_content' ) );
	...
	public function format_shortcode_content( $content ) {
		// delete p tag before and after content.
	...
	public function map_shortcodes( $key, $shortcode ) {
		...
		if ( $shortcode->get_is_parent_shortcode() ) {
			...
			add_filter(
				'qode_variation_swatches_for_woocommerce_filter_framework_add_shortcode_container',
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'qode-variation-swatches-for-woocommerce-framework-wpbakery', QODE_VARIATION_SWATCHES_FOR_WOOCOMMERCE_ADMIN_URL_PATH . '/inc/shortcodes/translators/wpbakery/assets/css/wpbakery.css' );
	}

	public function add_inline_style() {
		$shortcodes = qode_variation_swatches_for_woocommerce_framework_get_framework_root()->get_shortcodes()->get_shortcodes();
		$style      = apply_filters( 'qode_variation_swatches_for_woocommerce_filter_framework_add_wpbakery_inline_style', $style = '' );
		...
		if ( ! empty( $shortcodes ) && is_array( $shortcodes ) ) {
			...
			foreach ( $shortcodes as $key => $shortcode ) {
				...
				if ( isset( $shortcode_path ) && ! empty( $shortcode_path ) ) {
					...
					$style .= '.qvsfw-custom-vc-icon.' . $key . '{
		...
		if ( ! empty( $style ) ) {
			wp_add_inline_style( 'qode-variation-swatches-for-woocommerce-framework-wpbakery', $style );
		}
	}
}

if ( ! function_exists( 'qode_variation_swatches_for_woocommerce_framework_init_wpbakery_translator' ) ) {
	/**
	 * Function that initialize page builder module
	 */
	function qode_variation_swatches_for_woocommerce_framework_init_wpbakery_translator() {
		if ( qode_variation_swatches_for_woocommerce_is_installed( 'wpbakery' ) ) {
			new Qode_Variation_Swatches_For_WooCommerce_Framework_WPBakery_Translator();

			if ( class_exists( 'WPBakeryShortCodesContainer' ) ) {
				$classes = apply_filters( 'qode_variation_swatches_for_woocommerce_filter_framework_add_shortcode_container', array() );
	...
	add_action( 'init', 'qode_variation_swatches_for_woocommerce_framework_init_wpbakery_translator', 99 );
}
