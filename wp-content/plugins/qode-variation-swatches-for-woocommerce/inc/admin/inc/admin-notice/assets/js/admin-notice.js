(function ( $ ) {
	...
	var qodefDeactivationModal = {
		init: function () {
			this.deactivationLink  = $( '#the-list' ).find( '[data-slug="qode-variation-swatches-for-woocommerce"] span.deactivate a' );
			this.deactivationModal = $( '#qode-variation-swatches-for-woocommerce-deactivation-modal' );
		...
		initSubmitFunctionality: function () {
			var submitButton = this.deactivationModal.find( '.qodef-deactivation-modal-button-submit' ),
				...
				nonceHolder  = this.deactivationModal.find( '#qode-variation-swatches-for-woocommerce-deactivation-nonce' );
						...
						$.ajax(
							{
								type: 'POST',
								data: {
									action: 'qode_variation_swatches_for_woocommerce_deactivation',
									reason: reason,
