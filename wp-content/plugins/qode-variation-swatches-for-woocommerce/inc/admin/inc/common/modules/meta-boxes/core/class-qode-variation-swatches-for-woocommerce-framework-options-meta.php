...
class Qode_Variation_Swatches_For_WooCommerce_Framework_Options_Meta extends Qode_Variation_Swatches_For_WooCommerce_Framework_Options {
	private $has_meta_box;

	public function __construct() {
		parent::__construct();

		$this->set_has_meta_box( apply_filters( 'qode_variation_swatches_for_woocommerce_filter_has_meta_box_options', false ) );
		...
		if ( $this->get_has_meta_box() ) {
			...
			add_filter( 'sanitize_post_meta_qode_variation_swatches_for_woocommerce_meta_option', array( $this, 'sanitize_meta_option' ) );
			add_filter( 'sanitize_post_meta_qode_variation_swatches_for_woocommerce_meta_html_option', array( $this, 'sanitize_meta_html_option' ) );
	...
	public function populate_meta_box() {
		do_action( 'qode_variation_swatches_for_woocommerce_action_framework_populate_meta_box' );
	}

	public function meta_box_register() {

		do_action( 'qode_variation_swatches_for_woocommerce_action_framework_before_meta_options_register' );
		...
		do_action( 'qode_variation_swatches_for_woocommerce_action_framework_after_meta_options_register' );
	}
	...
	public function render_meta_box( $post, $metabox ) {
		...
		qode_variation_swatches_for_woocommerce_framework_template_part( QODE_VARIATION_SWATCHES_FOR_WOOCOMMERCE_ADMIN_PATH, 'inc/common', 'modules/meta-boxes/templates/holder', '', $params );
	}
	...
	public function remove_default_custom_fields() {
		$post_types = apply_filters( 'qode_variation_swatches_for_woocommerce_filter_framework_meta_box_remove', array() );
	...
	public function meta_box_save( $post_id, $post ) {
		...
		if ( is_array( $meta_boxes ) && count( $meta_boxes ) ) {
			foreach ( $meta_boxes as $meta_box ) {
				$nonces_array[] = 'qode_variation_swatches_for_woocommerce_framework_meta_box_' . $meta_box->get_slug() . '_save';
			}
		}
		...
		$post_types = apply_filters( 'qode_variation_swatches_for_woocommerce_filter_framework_meta_box_save', array( 'product' ) );
		...
		foreach ( $this->get_options() as $key => $box ) {
			...
			if ( array_key_exists( $key, $_POST ) ) {
				// Check if the field should be sanitized as wp_editor.
				$wp_kses_html_fields = apply_filters( 'qode_variation_swatches_for_woocommerce_filter_framework_wp_kses_html_fields', array() );

				if ( in_array( $key, $wp_kses_html_fields, true ) ) {
					$value = sanitize_meta( 'qode_variation_swatches_for_woocommerce_meta_html_option', array_key_exists( $key, $_POST ) ? wp_unslash( $_POST[ $key ] ) : '', 'post' );
				} else {
					$value = sanitize_meta( 'qode_variation_swatches_for_woocommerce_meta_option', array_key_exists( $key, $_POST ) ? wp_unslash( $_POST[ $key ] ) : '', 'post' );
				}
			}
	...
	public function sanitize_meta_html_option( $value ) {
		$sanitized_value = '';
		$trim_value      = ! is_array( $value ) ? trim( $value ) !== '' : ! empty( array_filter( $value ) );

		if ( ( ! empty( $value ) || '0' === $value || 0 === $value ) && $trim_value ) {
			$sanitized_value = qode_variation_swatches_for_woocommerce_framework_wp_kses_html( 'content', $value );
		}
	...
	public function enqueue_framework_meta_scripts() {
		...
		if ( function_exists( 'get_current_screen' ) && get_current_screen()->base === 'post' && in_array( get_current_screen()->post_type, $post_types, true ) ) {
			...
			do_action( 'qode_variation_swatches_for_woocommerce_action_framework_page_additional_scripts' );
		}
	}
