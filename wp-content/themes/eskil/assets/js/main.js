(function ( $ ) {
	...
	$( document ).ready(
		function () {
			...
			qodefAnchor.init();
			qodefOverlayHover.init();
			qodefThemeButton.init();
		}
	);
			...
			return {
				on: {
					...
					init: function () {
						...
						qodef.body.trigger(
							'eskil_trigger_swiper_is_initialized',
							[$holder, options]
						);
	...
	var qodefSideFilter = {
		init: function() {
			var countHolder      = $( '.wc-block-product-categories .wc-block-product-categories-list--depth-0, .woocommerce-widget-layered-nav-list' ),
				showMoreAppend   = '<a href="#" class="qodef-filter-show-more">More filters</a><a href="#" class="qodef-filter-show-more qodef-show-less">Show less</a>',
				itemClasses      = '.wc-block-product-categories-list-item, .wc-layered-nav-term',
				countHolderItems = countHolder.find( itemClasses );

			if ( countHolder.length ) {
				countHolder.each(
					function() {
						if ( countHolderItems.length > 8 ) {
							$( this ).append( showMoreAppend );
						}
					}
				);
			}

			qodefSideFilter.qodefSideFilterShowMore( countHolder );
		},
		qodefSideFilterShowMore: function( $countHolder ) {
			// TODO: height calculation for hierarchical items
			var countHolder = $countHolder;

			if ( countHolder.length ) {
				countHolder.each( function() {
					var $thisList             = $( this ),
						itemClasses           = '.wc-block-product-categories-list-item, .wc-layered-nav-term',
						itemChildClasses      = '.wc-block-product-categories-list-item > a, .wc-layered-nav-term > a',
						countHolderItems      = $thisList.find( itemClasses ),
						countHolderChildItems = $thisList.find( itemChildClasses );

					if ( countHolderItems.length > 7 ) {
						var countHolderItemsHeight = countHolderChildItems.outerHeight(true),
							holderHeight           = countHolderChildItems.toArray().reduce( (curr, next, index) => {
								if ( index < 8 ) {
									return curr + $(next).outerHeight( true );
								}
								return curr;
							},20),
							holderHeightInit       = countHolderItemsHeight * countHolderItems.length + 40 + 'px';

						if ( countHolderItems.length > 8 ) {
							$thisList.addClass( 'qodef-show-show-more' );
						}

						$thisList.css( "height", holderHeight );

						$thisList.find( '.qodef-filter-show-more' ).on(
							'click', function ( e ) {
								e.preventDefault();
								var $btn = $( this );

								$thisList.toggleClass('qodef-show-show-more--opened');

								if ( $btn.hasClass( 'qodef-show-less' ) ) {
									$thisList.animate( { height: holderHeight }, 300 );
									$btn.fadeOut(
										0,
										function () {
											setTimeout( function () {
												$btn.siblings( '.qodef-filter-show-more' ).fadeIn( 300 );
											}, 300 );
										}
									);
								} else {
									...
									$thisList.animate( { height: holderHeightInit }, 300 );
								}
							}
						);
					}
	...
	var qodefOverlayHover = {
		init: function () {
			var items = $('.qodef-overlay-on-hover');
	...
	qodef.qodefSideFilter = qodefSideFilter;
	
	var qodefThemeButton = {
		init: function () {
			var items = $('.qodef-theme-button.qodef-layout--text-appear');
			
			if ( items.length ) {
				items.each( function () {
					var item       = $( this ),
						$text      = item.find( '.qodef-m-text' ),
						$textWidth = Math.floor( $text.outerWidth() );
					
					$text.width( 0 );
					item.width( 24 );
					item.addClass( 'qodef--init' );
					
					item.on(
						'mouseenter',
						function () {
							item.width( $textWidth + 30 );
	...
	qodef.qodefThemeButton = qodefThemeButton;
	...
	var qodefMasonryLayout = {
				...
				function () {
					if ( typeof $masonry.isotope === 'function' ) {
						...
						if ( $holder.hasClass( 'qodef-items--fixed' ) ) {
							var size = qodefMasonryLayout.getFixedImageSize(
								$masonry,
								$masonryItem
							);

							if ( $holder.hasClass( 'qodef-woo-product-list' ) ) {
								size = Math.round( size ); // Product list rounding order fix
							}
		...
		setFixedImageProportionSize: function ( $holder, $item, size ) {
			var padding         = parseInt(
				...
				isMobileScreen  = qodef.windowWidth <= 820; // increased from 680 for other screen sizes
...
(function ($) {
	...
	$(window).on(
		...
		function () {
			...
			qodefWooDropdownOverlay.init();
		}
	);

	var qodefProductBehindHeader = {
		init: function () {

			var $productSingle = $( '.qodef-product-layout--slider, .qodef-product-layout--full-width' ),
				$headerTop = $productSingle.find($('#qodef-top-area')),
				$headerTopHeight = $headerTop.outerHeight() ? $headerTop.outerHeight() : 0,
				$header = $productSingle.find( $('#qodef-page-header')),
				$headerHeight = $header.outerHeight() ? $header.outerHeight() : 0,
				$inner = $productSingle.find($('#qodef-page-inner')),
				$titleHeight = $productSingle.find($('.qodef-page-title')).outerHeight() ? $productSingle.find($('.qodef-page-title')).outerHeight() : 0;

			if ( $productSingle.length && $( window ).width() > 1024 ) {
				$inner.css('margin-top', - ( $headerHeight + $headerTopHeight + $titleHeight ) );
			}
		}
	}
	...
	var qodefWooDropdownOverlay = {
		init: function () {
			var $holder  = $('.select2.select2-container'),
				$wrapper = $(' #qodef-page-wrapper '),
				$body    = $( 'body' );

			$( '.qodef-dropdown-cover' ).remove();
			$wrapper.prepend( '<div class="qodef-dropdown-cover"/>' );
			$body.removeClass( 'qodef-dropdown-select-opened' );

			if($holder.length) {
				$holder.on(
					'click',
					function() {
						$body.toggleClass( 'qodef-dropdown-select-opened' );
					}
				);
			}

			$( '.qodef-dropdown-cover' ).on(
				'click',
				function() {
					$body.removeClass( 'qodef-dropdown-select-opened' );
				}
